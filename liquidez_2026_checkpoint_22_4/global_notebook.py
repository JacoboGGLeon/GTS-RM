...
class GlobalNotebookConfig:
    ...
    calendar_uri: str
    artifact_root: str
    # ``horizon`` conserva el nombre interno histórico, pero ahora representa
    # exclusivamente el tamaño del bloque que el modelo aprende y emite por
    # ``forward``. La superficie del notebook lo llama ``rollout_chunk_size``.
    horizon: int = 25
    # Número total máximo de pasos solicitados a la inferencia por ``n_steps``.
    # Puede ser mayor que ``horizon``: el forecast repetirá varios bloques.
    ...
    artifact_s3_uri: str = ""
    model_config: GTRMModelConfig = field(default_factory=GTRMModelConfig)
    gtrm_stage: int = 2

    def validate(self) -> None:
        architecture = str(self.architecture).strip().lower()
        if isinstance(self.gtrm_stage, bool) or int(self.gtrm_stage) <= 0:
            raise ValueError("gtrm_stage must be a positive integer")
        self.model_config.validate(stage=int(self.gtrm_stage))
        if self.model_config.normalized_architecture() != architecture:
            raise ValueError(
                "GlobalNotebookConfig.architecture and model_config.architecture must match"
            )
        ...
        if self.horizon > self.forecast_horizon:
            raise ValueError("horizon/rollout_chunk_size cannot exceed forecast_horizon")
        if self.seen_validation_size < self.horizon:
            raise ValueError(
                "seen_validation_size must be at least horizon/rollout_chunk_size"
            )
    ...
    def rollout_chunk_size(self) -> int:
        """Alias semántico del ``horizon`` interno del modelo."""

        return int(self.horizon)

    def to_dict(self) -> Mapping[str, Any]:
        payload = asdict(self)
        payload["rollout_chunk_size"] = int(self.horizon)
        return payload
...
class GlobalNotebookDatasetFactory:
    ...
    def __init__(
        ...
        *,
        ...
        if model_config is not None:
            model_config.validate(stage=2)
            use_static_context = model_config.use_static_context
        if not isinstance(use_static_context, bool):
            raise TypeError("use_static_context must be a boolean")

        self.model_config = model_config or GTRMModelConfig(use_static_context=bool(use_static_context))
        self.use_static_context = bool(self.model_config.use_static_context)
        # ``horizon`` es la dimensión de salida del modelo por bloque. La
        # superficie pública lo denomina ``rollout_chunk_size`` para no
        # confundirlo con el horizonte total solicitado en inferencia.
    ...
    def summary(self) -> Mapping[str, Any]:
        return {
            ...
            "horizon": self.horizon,
            "rollout_chunk_size": self.horizon,
            "seen_validation_size": self.seen_validation_size,
            "stride": self.stride,
            "max_window_size": self.max_window_size,
            ...
            "event_threshold": float(self.model_config.event_threshold),
            "magnitude_transform": str(self.model_config.magnitude_transform),
            "gtrm_model_config": self.model_config.as_dict(),
