...
class TestCheckpoint7ParameterizedGlobalNotebook(unittest.TestCase):
    def test_canonical_global_notebook_preserves_parameter_cell_without_outputs(self) -> None:
        ...
        self.assertIn("workflow.run_hpo_and_train(", source)
        self.assertIn("workflow.run_backtest(", source)
        self.assertIn("workflow.run_forecast(", source)
        self.assertNotIn("manager.fit_global(", source)
            ...
            GlobalNotebookConfig(
                **{
                    **valid.__dict__,
                    ...
                    "model_config": GTRMModelConfig(architecture="transformer"),
                }
            ).validate()
    ...
    def test_load_inputs_validates_real_notebook_contract(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            config = GlobalNotebookConfig(
                ...
                n_trials=1,
                max_window_size=3,
                model_config=GTRMModelConfig(architecture="rnn_bi"),
            )
