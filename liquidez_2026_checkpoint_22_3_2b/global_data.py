...
WINDOW_TARGET_FIELDS: Final[Tuple[str, ...]] = (
    "y_future",
    "y_future_raw",
    "event_target",
    "magnitude_target",
...
class StaticFeatureEncoder:
    ...
    Checkpoint 21 permite apagar el contexto estático para ejecutar la ablation
    GTRM-A sin cambiar el contrato de ``forward``. En ese modo se emite un
    vector cero de dimensión 1, de modo que ``x_static`` sigue existiendo pero
    no transporta información semántica ni identificadora.
    """
    ...
    @classmethod
    def disabled(cls) -> "StaticFeatureEncoder":
        """Crea encoder nulo para ``use_static_context=False``."""
    ...
    def feature_names(self) -> Tuple[str, ...]:
        if not self.enabled:
            return ("static_context_disabled",)
    ...
    def to_dict(self) -> Mapping[str, object]:
        return {
            ...
            "fit_scope": "training_series_only" if self.enabled else "disabled_by_use_static_context_false",
            "continuous_features": {
...
def compute_scaled_movement(
    y_context: np.ndarray,
    y_future: np.ndarray,
    *,
    series_type: str,
) -> np.ndarray:
    """Movimiento agnóstico para heads auxiliares.

    Para ``saldo`` el movimiento relevante es el cambio contra el último valor
    observado del contexto. Para ``variacion`` el target ya representa cambio,
    por lo que se usa directamente. Todo opera en escala causal de la ventana.
    """

    context = np.asarray(y_context, dtype=np.float32).reshape(-1, 1)
    future = np.asarray(y_future, dtype=np.float32).reshape(-1, 1)
    if context.size == 0 or future.size == 0:
        raise ValueError("y_context and y_future must be non-empty")
    normalized_type = str(series_type).strip().lower()
    if normalized_type == "saldo":
        reference = context[-1]
        return future - reference
    if normalized_type == "variacion":
        return future
    return future


def build_agnostic_auxiliary_targets(
    y_context: np.ndarray,
    y_future: np.ndarray,
    *,
    series_type: str,
    ...
    """Targets derivados por ventana para heads agnósticos.

    No requiere etiquetas humanas. Los tres targets se calculan desde el futuro
    escalado causalmente y el movimiento definido por tipo de serie:

    - event_target: abs(movement_scaled) > threshold;
    - magnitude_target: asinh/log1p/abs(abs(movement_scaled));
    - direction_target: negativo/neutro/positivo codificado como 0/1/2.
    """

    if not np.isfinite(event_threshold) or float(event_threshold) <= 0.0:
        raise ValueError("event_threshold must be a positive finite number")
    movement = compute_scaled_movement(y_context, y_future, series_type=series_type)
    abs_movement = np.abs(movement).astype(np.float32)
    event = (abs_movement > float(event_threshold)).astype(np.float32)
    transform = str(magnitude_transform or "asinh").strip().lower()
    if transform == "asinh":
        magnitude = np.arcsinh(abs_movement).astype(np.float32)
    elif transform == "log1p":
        magnitude = np.log1p(abs_movement).astype(np.float32)
    elif transform in {"abs", "none", "identity"}:
        magnitude = abs_movement.astype(np.float32)
    else:
        raise ValueError("magnitude_transform must be one of: asinh, log1p, abs")
    direction = np.ones(movement.shape[0], dtype=np.int64)
    flat = movement.reshape(-1)
    direction[flat < -float(event_threshold)] = 0
    direction[flat > float(event_threshold)] = 2
    return {
        "movement_scaled": movement.astype(np.float32),
        "event_target": event.astype(np.float32),
        "magnitude_target": magnitude.astype(np.float32),
...
class GlobalWindowDataset(Dataset[Mapping[str, object]]):
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(use_static_context, bool):
            raise TypeError("use_static_context must be a boolean")
        self.use_static_context = bool(use_static_context)
        if not np.isfinite(event_threshold) or float(event_threshold) <= 0.0:
            raise ValueError("event_threshold must be a positive finite number")
        self.event_threshold = float(event_threshold)
        self.magnitude_transform = str(magnitude_transform or "asinh").strip().lower()
        if self.magnitude_transform not in {"asinh", "log1p", "abs", "none", "identity"}:
            raise ValueError("magnitude_transform must be one of: asinh, log1p, abs")
        self.scaler = scaler or ContextScaler()
        self.static_feature_encoder = (
            (static_feature_encoder or StaticFeatureEncoder.fit(global_long))
            if self.use_static_context
            else StaticFeatureEncoder.disabled()
        )
    ...
    def __getitem__(self, index: int) -> Mapping[str, object]:
        ...
        auxiliary_targets = build_agnostic_auxiliary_targets(
            y_context,
            y_future,
            series_type=series.series_type,
            event_threshold=self.event_threshold,
            magnitude_transform=self.magnitude_transform,
        )
        return {
            ...
            "targets": {
                ...
                "event_target": torch.as_tensor(auxiliary_targets["event_target"], dtype=self.tensor_dtype),
                "magnitude_target": torch.as_tensor(auxiliary_targets["magnitude_target"], dtype=self.tensor_dtype),
                "direction_target": torch.as_tensor(auxiliary_targets["direction_target"], dtype=torch.long),
            },
            "metadata": {
