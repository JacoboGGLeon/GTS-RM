...
El manager conecta HPO proxy, warm-up, fine-tuning curricular, evaluación,
forecast y persistencia de un único ``state_dict`` compartido. Checkpoint 10
...
ARTIFACT_SCHEMA_VERSION: Final[str] = "1.4"
...
class GlobalManager:
    ...
    def fit_global(
        ...
        *,
        ...
        """Ejecuta HPO, warm-up y fine-tuning continuo del modelo global."""
    ...
    def _finetune_all(
        ...
        *,
        ...
        if session is None:
            raise RuntimeError("Run _warmup_all() before _finetune_all()")
    ...
    def visualise(
        ...
        *,
        ...
        if not self._backtest_results:
            raise RuntimeError("Run _run_backtest() before visualise()")
        if not self._future_results:
            raise RuntimeError("Run _run_forecast() before visualise()")
    ...
    def save_model(self, artifact_dir: str | os.PathLike[str]) -> Path:
        ...
        try:
            ...
            manifest: MutableMapping[str, Any] = {
                ...
                "curriculum_config": _jsonable(asdict(training.curriculum_config)),
    ...
    def load_model(
        ...
        *,
        ...
        curriculum_config = GlobalCurriculumConfig(**manifest["curriculum_config"])
        manager = cls(
            architecture,
