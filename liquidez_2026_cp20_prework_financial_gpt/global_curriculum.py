"""Warm-up y fine-tuning curricular con replay para modelos globales.

Checkpoint 5 extiende el entrenamiento global sin alterar HPO, arquitecturas,
notebooks ni orquestación. El modelo se construye exactamente una vez y conserva
pesos y estado del optimizador durante todo el recorrido:

1. warm-up con el nivel curricular más sencillo;
2. fine-tuning progresivo por nivel, mezclando replay de niveles previos;
3. consolidación final balanceada con todos los niveles.

``cross_key_id`` y ``nivel_curriculum`` se usan únicamente para sampling y
métricas. Nunca se entregan al ``forward`` del modelo.
"""
...
TRAINING_ORDERS: Final[Tuple[str, ...]] = ("curriculum", "shuffled")


@dataclass(frozen=True)
class GlobalCurriculumConfig:
    """Configuración de las fases posteriores al HPO global."""
    ...
    def validate(self) -> None:
        ...
        if self.training_order not in TRAINING_ORDERS:
            raise ValueError(
                f"training_order must be one of {TRAINING_ORDERS}"
            )

    def build_stages(self, curriculum_levels: Sequence[int]) -> Tuple["CurriculumStage", ...]:
        """Construye un camino único y acumulativo para los niveles disponibles."""
...
class GlobalCurriculumTrainingResult:
    """Modelo final después de warm-up, fine-tuning y consolidación."""
    ...
    def to_summary(self) -> Mapping[str, Any]:
        return {
            ...
            "curriculum_config": asdict(self.curriculum_config),
...
class GlobalCurriculumSession:
    """Estado continuo de warm-up y fine-tuning para un único modelo global.

    La sesión conserva el mismo modelo y el mismo optimizador entre fases. Esto
    permite exponer una ejecución paso a paso sin reiniciar pesos ni momentos de
    AdamW. Las fases deben ejecutarse en el orden definido por el curriculum.
    """
